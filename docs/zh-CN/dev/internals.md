...
这篇文档就是运行时地图。建议你一边阅读，一边对照 `src/kohakuterrarium/` 中的代码。`../concepts/` 下的文档解释的是为什么这样设计，这篇只说明内容放在哪里、流程如何运转。
...
更多内容见 [concepts/modules/tool.md](../concepts/modules/tool.md) 和 [impl-notes/stream-parser.md](../concepts/impl-notes/stream-parser.md)。
...
只要你修改了这些流程中的任意部分，都应该回来重新核对这些规则。
