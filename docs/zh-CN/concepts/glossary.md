...
框架层级的配置，把Creature回合结束的输出自动送到指定的目标。在Creature配置里用 `output_wiring:` 宣告；每一个回合结束时，框架把一个 `creature_output` TriggerEvent 直接推进指定的目标Creature的事件队列。不需要调用 `send_message`、也不经过频道，它走的是和其他触发器一样的事件路径。**确定性的 pipeline 边** 用输出接线；条件性、广播、观察类的流量留给频道。完整说明：[Terrarium使用指南的输出接线一节](../guides/terrariums.md#output-wiring)。
...
修改模块之间 **连接方式 ** 的代码，而不是 fork 某个模块。两种 ** ：prompt 插件 ** (为 system prompt 贡献内容) 与 **lifecycle 插件** (挂在 `pre_llm_call`、`post_tool_execute` 这类 hook)。`pre_*` hook 可以抛 `PluginBlockError` 来终止操作。完整说明：[插件](modules/plugin.md)。
...
主机或工作节点，即任何说 Lab 协议的进程。通过 `node_id`
