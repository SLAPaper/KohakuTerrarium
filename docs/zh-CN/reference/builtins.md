 ...
 **`notebook_read`** — 将 Jupyter notebook (`.ipynb`) 读取为紧凑 Markdown 摘要、指定 cell 或原始 JSON。直接执行。

- 参数：`path`、`cell`、`include_outputs`、`mode`。

 **`notebook_edit`** — 替换、插入、删除或追加 Jupyter notebook cell，并保留 notebook metadata。直接执行。

- 参数：`path`、`cell`、`source`、`cell_type`、`action`。
...
这些工具会变更或检查呼叫者所在的[图](../concepts/glossary.md#graph--图)。
分两层：

**基本** —— 注册在图中每只 Creature 上。工具内部按呼叫情境闸门
（例如 `group_send` 会对非特权呼叫者强制「目标必须是特权 Creature」
规则）：

- **`send_channel`** — 把消息广播到呼叫者已接线为 send 的频道。参数：
  `channel`、`content`。
- **`group_send`** — 直接点对点送给图中另一只 Creature（绕过频道）。
  非特权呼叫者只能指定特权目标（工人向主管回报，不能向同侪工人送）。
  参数：`target_creature_id`、`content`。

**特权** —— 仅注册在
[特权节点](../concepts/glossary.md#privileged-node--特权节点)上：

- **`group_add_node`** — 把一只 Creature 生成到呼叫者的图。参数：
  `config_path`（档案或 `@pkg/ref`）、可选 `name`、`llm`、`pwd`。回传
  新 Creature 的 id。
- **`group_remove_node`** — 停止并把 Creature 从图里移除。若它原是
  桥，可能触发[自动分裂](../concepts/glossary.md#auto-split--auto-merge--自动分裂--自动合并)。参数：`creature_id`。
- **`group_start_node`** / **`group_stop_node`** — 在不移除的前提下启
  动 / 停止 Creature。参数：`creature_id`。
- **`group_channel`** — 频道与 per-creature 接线的 CRUD。参数：
  `action` ∈ `create` | `delete` | `wire` | `unwire`、`channel`、可选
  `creature_id` 与 `direction`。跨图的 wire 会路由到
  `Terrarium.connect` 并可能自动合并。
- **`group_wire`** — 输出接线边的 CRUD。参数：`action` ∈ `add` |
  `remove`，加边规格（`from`、`to`、可选 `with_content`、`prompt`）。
- **`group_status`** — 呼叫者图的快照：Creature（连同 status、特权、
  父）、频道（连同 history pointer、连接的 Creature）、输出接线、可
  生成的 catalog。无参数。
...
所有内置子代理都会加载 `default_plugins: ["auto-compact"]`，并配置统一的 `budget` 插件作为最小运行时预算：turn 软/硬限制 `40/60`、工具调用软/硬限制 `75/100`，且没有 walltime 预算。
...
| `kimi-code` | `anthropic` | Kimi Code Anthropic-compatible endpoint | API-key 验证 (`KIMI_CODE_API_KEY`)。 |
| `glm-coding` | `anthropic` | GLM Coding Plan Anthropic-compatible endpoint | API-key 验证 (`GLM_CODING_API_KEY`)，使用 Bearer token。 |
...
Server 工具会在 system prompt 里以 `## Available MCP Tools` 章节呈现。Transport：`stdio`（子进程）、`streamable_http`（现代 HTTP MCP），以及旧式 `http`/`sse` SSE 端点。
