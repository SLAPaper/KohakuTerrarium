...
systemd ``LoadCredential=`` 指令传递，因此它们永远不会出现在
``/proc/<pid>/environ`` 中。
...
[反向代理部署](deployment-reverse-proxy.md)。当 ``auth`` 启用
但主机 URL 为明文 ``http://`` 且非 loopback 时，前端会显示横幅
*"连接未加密"*。

## 参见

- [Docker 部署](deployment-docker.md)：通过 ``secrets:`` 挂载
  传递 ``[auth]`` 配置
- [systemd 部署](deployment-systemd.md)：通过 ``LoadCredential=``
  指令传递 ``[auth]`` 配置
