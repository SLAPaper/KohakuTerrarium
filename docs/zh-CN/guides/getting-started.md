...
# 或安装可选 browser/demo/heavy-embedding 依赖
...
你会进入 SWE agent 的交互式提示环境。输入一个请求后，它会在目前工作目录中使用 shell、文件与编辑工具。Ctrl+C 会中断正在运行的 turn；空闲时按两次 Ctrl+C（或 Ctrl+D / `/exit`）可干净结束，并打印恢复提示。
...
- **`ModuleNotFoundError: pywebview`**。`pywebview` 是核心 PyPI 依赖的一部分；如果缺少它，请重新安装/升级 `kohakuterrarium`，或改用 `kt web`。
