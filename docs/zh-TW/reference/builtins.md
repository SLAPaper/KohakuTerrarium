...
**`notebook_read`** — 將 Jupyter notebook (`.ipynb`) 讀成精簡 Markdown 摘要、指定 cell 或原始 JSON。直接執行。

- 參數：`path`、`cell`、`include_outputs`、`mode`。

**`notebook_edit`** — 替換、插入、刪除或追加 Jupyter notebook cell，並保留 notebook metadata。直接執行。

- 參數：`path`、`cell`、`source`、`cell_type`、`action`。
...
這些工具會變更或檢查呼叫者所在的[圖](../concepts/glossary.md#graph--圖)。
分兩層：

**基本** —— 註冊在圖中每隻生物上。工具內部按呼叫情境閘門（例如
`group_send` 會對非特權呼叫者強制「目標必須是特權生物」規則）：

- **`send_channel`** — 把訊息廣播到呼叫者已接線為 send 的頻道。參數：
  `channel`、`content`。
- **`group_send`** — 直接點對點送給圖中另一隻生物（繞過頻道）。
  非特權呼叫者只能指定特權目標（工人向主管回報，不能向同儕工人送）。
  參數：`target_creature_id`、`content`。

**特權** —— 僅註冊在
[特權節點](../concepts/glossary.md#privileged-node--特權節點)上：

- **`group_add_node`** — 把一隻生物生成到呼叫者的圖。參數：
  `config_path`（檔案或 `@pkg/ref`）、可選 `name`、`llm`、`pwd`。回傳
  新生物的 id。
- **`group_remove_node`** — 停止並把生物從圖裡移除。若它原是橋，可能
  觸發[自動分裂](../concepts/glossary.md#auto-split--auto-merge--自動分裂--自動合併)。參數：`creature_id`。
- **`group_start_node`** / **`group_stop_node`** — 在不移除的前提下啟
  動 / 停止生物。參數：`creature_id`。
- **`group_channel`** — 頻道與 per-creature 接線的 CRUD。參數：
  `action` ∈ `create` | `delete` | `wire` | `unwire`、`channel`、可選
  `creature_id` 與 `direction`。跨圖的 wire 會路由到
  `Terrarium.connect` 並可能自動合併。
- **`group_wire`** — 輸出接線邊的 CRUD。參數：`action` ∈ `add` |
  `remove`，加邊規格（`from`、`to`、可選 `with_content`、`prompt`）。
- **`group_status`** — 呼叫者圖的快照：生物（連同 status、特權、
  父）、頻道（連同 history pointer、連接的生物）、輸出接線、可生成
  的 catalog。無參數。
...
所有內建子代理都會載入 `default_plugins: ["auto-compact"]`，並設定統一的 `budget` 外掛作為最小執行期預算：turn 軟/硬限制 `40/60`、工具呼叫軟/硬限制 `75/100`，且沒有 walltime 預算。
...
| `kimi-code` | `anthropic` | Kimi Code Anthropic-compatible endpoint | API-key 驗證 (`KIMI_CODE_API_KEY`)。 |
| `glm-coding` | `anthropic` | GLM Coding Plan Anthropic-compatible endpoint | API-key 驗證 (`GLM_CODING_API_KEY`)，使用 Bearer token。 |
...
Server tool 會在系統提示裡以 `## Available MCP Tools` 出現。Transport：`stdio`（子行程）、`streamable_http`（現代 HTTP MCP），以及舊式 `http`/`sse` SSE 端點。
