...
> **推荐 Python 版本**：Python 3.12 及以上。CI 只验证 3.12+；3.10/3.11 仍可安装运行（`requires-python = ">=3.10"`），但属于 best-effort —— 旧版 asyncio + SQLite daemon thread 交互较慢，集成测试会偶发 timeout。
...
# 或内置 API-key 提供者：`kt config key set <provider>`
```

支持 Codex OAuth、OpenRouter/OpenAI、原生 Anthropic、Google Gemini、Kimi Code、GLM Coding Plan，以及任何 OpenAI 兼容 API。
