...
- **完整的內建工具與子代理。** 檔案、shell、網頁、JSON、notebook/Jupyter、搜尋、編輯、規劃、審查、研究、生態瓶管理。
- **MCP 支援。** 可以按生物或全域連接 stdio、streamable HTTP 或舊式 SSE/HTTP MCP 伺服器；工具會自動出現在 prompt 裡。
...
> **建議 Python 版本**：Python 3.12 以上。CI 只驗證 3.12+；3.10/3.11 仍可安裝執行 (`requires-python = ">=3.10"`)，但屬於 best-effort — 舊版 asyncio + SQLite daemon thread 互動較慢，整合測試會偶發 timeout。
...
- [部署 — Docker](docs/zh-TW/guides/deployment-docker.md) — AIO、host + workers、分散式 compose
- [部署 — systemd](docs/zh-TW/guides/deployment-systemd.md) — `kt service install` + 強化過的 unit
- [部署 — 反向代理](docs/zh-TW/guides/deployment-reverse-proxy.md) — nginx / Cloudflare Tunnel + TLS
...
- **子代理** 是單一生物內部的縱向 / 私有委派。當一個控制器能在內部拆解任務時優先使用子代理；當多個平級生物需要橫向合作時使用 Terrarium。

### 頻道與輸出接線

頻道和輸出接線構成生物之間的橫向合作機制：

- **Queue** — 每則訊息被一個消費者收到。
- **Broadcast** — 每則訊息被所有訂閱者收到。
- **輸出接線 (output wiring)** — 確定性的 pipeline 邊，會把某個生物回合結束輸出自動送到指定目標。
...
- 檔案、shell、網頁、JSON、notebook/Jupyter、頻道、觸發器、內省工具，含單次編輯與多點編輯原語。
...
- MCP (Model Context Protocol) 整合 — stdio、streamable HTTP 與舊式 SSE/HTTP 傳輸。
...
`examples/deployment/` 下備有可立即套用的 compose 檔 (AIO、host + workers、分散式) 與 nginx TLS 終止範本。`/healthz` + `/readyz` 端點供 Docker `HEALTHCHECK` 與反向代理 active health 使用。

見 [部署 — Docker](docs/zh-TW/guides/deployment-docker.md)、[部署 — systemd](docs/zh-TW/guides/deployment-systemd.md)、[部署 — 反向代理](docs/zh-TW/guides/deployment-reverse-proxy.md)。
...
### 使用指南
[快速開始](docs/zh-TW/guides/getting-started.md) · [撰寫生物](docs/zh-TW/guides/creatures.md) · [生態瓶](docs/zh-TW/guides/terrariums.md) · [工作階段](docs/zh-TW/guides/sessions.md) · [記憶](docs/zh-TW/guides/memory.md) · [設定檔](docs/zh-TW/guides/configuration.md) · [程式化使用](docs/zh-TW/guides/programmatic-usage.md) · [組合](docs/zh-TW/guides/composition.md) · [自訂模組](docs/zh-TW/guides/custom-modules.md) · [外掛](docs/zh-TW/guides/plugins.md) · [MCP](docs/zh-TW/guides/mcp.md) · [套件](docs/zh-TW/guides/packages.md) · [Serving](docs/zh-TW/guides/serving.md) · [Laboratory](docs/zh-TW/guides/laboratory.md) · [部署 — Docker](docs/zh-TW/guides/deployment-docker.md) · [部署 — systemd](docs/zh-TW/guides/deployment-systemd.md) · [部署 — 反向代理](docs/zh-TW/guides/deployment-reverse-proxy.md) · [範例](docs/zh-TW/guides/examples.md)
