...
// Session-shell renderers: the icon/action-button atoms and the Session rows.
// Kept free of the shared chat-ui package so the shell controls stay unit
// testable without the build alias (the transcript components live next door).
export function createSessionRenderers({ available, busy, currentSession, openSession, resumeSession }) {
  function icon(name) {
    return h(
      'svg',
      {
  ...
  function actionButton(label, iconName, { text, ...options } = {}) {
    return h('button', { type: 'button', title: label, 'aria-label': label, ...options }, [icon(iconName), text ? h('span', text) : null])
  }
    ...
    return h(
      ...
      [
        h(
          'div',
          {
            class: ['session-row', 'session-row--label', selected ? 'is-active' : ''],
          },
          [h('span', { class: 'status-dot', 'aria-hidden': 'true' }), h('span', { class: 'row-label' }, session.title)],
        ),
  ...
  return { actionButton, icon, renderSession }
}
